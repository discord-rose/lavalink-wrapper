...
class LavalinkManager extends typed_emitter_1.EventEmitter {
    ...
    async search(query, requester, source = this.options.defaultSource) {
        ...
        if (this.spotifyToken && ['album', 'playlist', 'track'].includes(spotifyMatch[1])) {
            ...
            if (spotifyMatch[1] === 'album' || spotifyMatch[1] === 'playlist') {
                const res = await node_fetch_1.default(`${Constants_1.default.SPOTIFY_BASE_URL}/${spotifyMatch[1]}s/${spotifyMatch[2]}`, {
                    method: 'GET', headers
                });
    ...
    async _renewSpotifyToken() {
        ...
        const res = await node_fetch_1.default(Constants_1.default.SPOTIFY_TOKEN_ENDPOINT, {
            method: 'POST',
            headers,
